 ...
 public:
  bool set_light_color_values(light::LightColorValues v) {
    ...
    // this firmware, as the original firmware does not support it.
      ...
      red = std::lerp(0.9997f, 0.960f, v.get_red()); // 0.9997f, 0.9680f
      green = std::lerp(0.9997f, 0.960f, v.get_green()); // 0.9997f, 0.9680f
      auto blue_scale = (v.get_red() + v.get_green()) / 2.0f;
      auto blue_max = std::lerp(0.960f, 0.960f, blue_scale); //0.9640f, 0.9720f
      blue = std::lerp(0.9997f, blue_max, v.get_blue()); //0.9997f
      white = 0.0f;
    }
